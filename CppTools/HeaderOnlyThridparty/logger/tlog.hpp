...
#define tlog(level, format, ...)                                               \
  tlog_ext(level, BASE_FILE_NAME, __LINE__, __func__, NULL, format,            \
           ##__VA_ARGS__)
...
public:
  ...
  static std::string tlog_get_local_time() {
    ...
    sprintf(tmp2, "%d", tb.millitm);
    std::string time_srt1 = tmp;
    std::string time_srt2 = tmp2;
    return (time_srt1 + time_srt2);
  };
  ...
  static inline const char *level_str(const tlog_level level) {
    switch (level) {
    case 0:
      return ("DEBUG");
    case 1:
