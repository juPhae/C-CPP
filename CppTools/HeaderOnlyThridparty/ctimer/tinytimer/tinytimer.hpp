...
public:
  ...
  //! @return 执行结果
  //!
  bool SetTimer(int interval, std::function<void()> task, bool bLoop = false,
                bool async = true) {
    ...
    if (async) {
      ...
      m_Thread = std::make_shared<std::thread>(([this, interval, task]() {
        while (!m_bTryExpired) {
          ...
          if (m_ExpiredConditionVar.wait_for(
                  lk, std::chrono::milliseconds(interval),
                  [this]() { return m_bTryExpired == true; })) {
            break;
          } else {
            task();
  ...
  bool SyncOnceExecute(int interval, callable &&fun, arguments &&... args) {
    std::function<typename std::result_of<callable(arguments...)>::type()> task(
        std::bind(
            ...
            std::forward<arguments>(args)...)); //定任务函数或lambda成function
